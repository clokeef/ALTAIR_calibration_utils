...
def build_corr_table(corr):
    """
    puts together an Astropy table of the correlation files data
    :param corr: correlation file from astrometry.net plate solution
    :return: Astropy Table of the correlation data
    """
    hd = corr[1].header
    data = corr[1].data
...
def get_magzero_est(corr,sim, file_path, file_name, read_in = False):
    """
    estimates the magnitude zero-point for an image given an astrometry.net corr file
    :param corr: correlation file produced by astrometry.net plate solution
    :param sim: background subbed streak_interface instance of the associated image
    :return: median magnitude zero-point, array of calculated magnitude zero-points
    """
    if read_in == True:
